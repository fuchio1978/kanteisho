...
test('六柱推命モードは半方合を半会より先に確定する', () => {
  ...
  assert.equal(dayBranch.wood, 1);
  assert.equal(dayBranch.fire, 0);
  assert.ok(result.states[1].relations.some(relation => relation.label === '半方合'));
  assert.equal(result.states[1].relations.some(relation => relation.label === '半会'), false);
...
test('東西南北の方合・三合局・疑似局は変化可能枠へ優先順位順に作用する', () => {
  ...
  assert.equal(pseudo.notes[0].startsWith('疑似局'), true);
...
test('申亥辰の疑似水局は中心の亥1枠で申と辰を各水1にし辰の無作用2を残す', () => {
  ...
  assert.equal(pseudo.notes[0].startsWith('疑似局(申・亥・辰)→water'), true);
...
test('半方合と半会が重なる場合は半方合を優先して受け枠を越えない', () => {
  ...
  assert.equal(scores.water, 0);
  assert.equal(scores.wood, 1);
