...
test('十二支の蔵干は指定された順番を保つ', () => {
  const expected = {
    子: '壬癸', 丑: '癸辛己', 寅: '戊丙甲', 卯: '甲乙', 辰: '乙癸戊', 巳: '戊庚戊',
...
test('六柱推命モードの年運候補は0歳から120歳を年齢・大運付きで表示する', () => {
  const base = calculate('1978-07-24', '19:40', 16, '男性');
...
test('東西南北の方合・三合局・疑似局は変化可能枠へ優先順位順に作用する', () => {
  ...
  assert.equal(context.api.branchStateScores(pseudo.states[2]).fire, 3);
});
...
test('戌辰午子は冲の相手以外へ半会と土化を続け残量も保持する', () => {
  ...
  assert.equal(result.states[2].remainingTransformCapacity, 2.5);
