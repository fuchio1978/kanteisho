...
test('会員版は受け取った権限からプラン別の表示範囲を決める', () => {
  ...
  assert.equal(context.api.memberFeatureVisibility(['compatibility']).compatibility, true);
});

test('相性鑑定は保存した入力から通常画面と同じ命式を再現する', () => {
  const subject={display_name:'比較対象',birth_year:1978,birth_month:7,birth_day:4,birth_hour:19,birth_minute:40,birth_time_unknown:false,sex:'男性',birthplace_label:'新潟',local_offset_minutes:16,standard_longitude:135,hemisphere:'north'};
...
test('通常年運も六柱推命の選択年を中心に前後5年を表示する', () => {
  assert.deepEqual(Array.from(context.api.annualWindowYears(1996)), [1991,1992,1993,1994,1995,1996,1997,1998,1999,2000,2001]);
  const source = fs.readFileSync(require('node:path').join(__dirname, '..', 'app.js'), 'utf8');
  assert.match(source, /renderAnnual\(x,p,input\.value\)/);
});

test('鑑定書ヘッダーは生まれ時間と出身地を表示する', () => {
  const source = fs.readFileSync(require('node:path').join(__dirname, '..', 'app.js'), 'utf8');
  assert.match(source, /生まれ時間 \$\{birthTime\}　出身地 \$\{x\.birthplace/);
  const css = fs.readFileSync(require('node:path').join(__dirname, '..', 'styles.css'), 'utf8');
  assert.match(css, /\.pdf-report-person span\{[^}]*white-space:pre-line/);
});
...
test('推命気温は五行の強中弱と月支温度を表どおり合算する', () => {
  ...
  assert.deepEqual({ ...allStrong.contributions }, { wood: 2, fire: 4, earth: 0, metal: -2, water: -4, monthBranch: 2 });
  assert.equal(allStrong.total, 2);

  const mixed = context.api.destinyTemperature(
    { wood: '中', fire: '強', earth: '強', metal: '中', water: '弱' }, ['卯'],
  );
  assert.deepEqual({ ...mixed.contributions }, { wood: 1, fire: 4, earth: -2, metal: -1, water: 0, monthBranch: 0 });
  assert.equal(mixed.total, 2);

  const allWeak = context.api.destinyTemperature(
    { wood: '弱', fire: '弱', earth: '弱', metal: '弱', water: '弱' }, ['亥'],
  );
  assert.equal(allWeak.total, -2);
});

test('六柱の推命気温は大運支を月支と同じ温度補正に加え、年運支は加えない', () => {
  const pillars = {
    year: ['丙', '子'], month: ['丁', '巳'], day: ['戊', '丑'], hour: ['己', '申'],
  };
  const natal = context.api.natalElementScores(pillars);
  const six = context.api.sixElementScores(pillars, ['甲', '亥'], ['乙', '午']);
  assert.deepEqual([...natal.temperature.majorBranches], ['巳']);
...
test('景色番号117は日主別の用神・景色・理由と共通事項だけを返す', () => {
  ...
  assert.equal(bing.label, '（冬月）甲（木）／それ以外は庚（岩石）');
  assert.equal(bing.landscape, '湖のある山に沈む大きな夕日');
  assert.match(bing.reason, /^寒い季節は木/);
  assert.doesNotMatch(`${bing.label}${bing.landscape}${bing.reason}`, /丁/);
  assert.deepEqual([...bing.stems], ['甲', '庚']);

  const ding = context.api.useGodFromStrengths(strengths, '丁', '亥');
  assert.equal(ding.number, 117);
  assert.equal(ding.label, '甲（木）＋庚（岩石）');
  assert.match(ding.landscape, /^泥の上で燃える灯火/);
  assert.match(ding.reason, /^木で土の力/);
  assert.doesNotMatch(`${ding.label}${ding.landscape}${ding.reason}`, /丙/);
  assert.deepEqual([...ding.stems], ['甲', '庚']);
});
...
test('丙が干合化水して壬になると日主丁との近貼を取り消す', () => {
  const pillars = { hour: ['庚', '戌'], day: ['丁', '酉'], month: ['己', '亥'], year: ['辛', '酉'] };
  const result = context.api.resolveSixPillarFiveElements(pillars, ['庚', '子'], ['丙', '子']);
  assert.equal(result.stemElements[5], 'water');
  assert.ok(result.stemStamps[5].includes('干合'));
  assert.equal(result.stemStamps[5].includes('近貼'), false);
  assert.equal(result.stemStamps[1].includes('近貼'), false);
  assert.equal(context.api.sixPillarModel(pillars, ['庚', '子'], ['丙', '子'], result)[5].cells[0].transformedChar, '壬');
});
