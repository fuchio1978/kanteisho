...
function pdfFortuneCycleContext(x,p,year){
  ...
  const annualStates=annualValues.map((value,index)=>{const active=[...luck.cycles].reverse().find(cycle=>cycle.start.year<=years[index]),luckValue=active?[active.stem,active.branch]:null,luckState=luckValue?resolveDownstreamBranch(natal.states,natalValues,luckValue,'major',p.month[1]):null;return resolveDownstreamBranch(luckState?[...natal.states,luckState]:natal.states,luckValue?[...natalValues,luckValue]:natalValues,value,'minor',p.month[1])});
