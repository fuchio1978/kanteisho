...
function applyNearbyStemStamps(values,stemResolution){const dayStem=values[1]?.[0],dayIndex=STEMS.indexOf(dayStem),dayElement=stemResolution.elements[1];if(dayIndex%2!==1)return;const neighbors=values.length>4?[0,2,...values.map((_,index)=>index).filter(index=>index>=4)]:[0,2];for(const neighbor of neighbors){const neighborStem=values[neighbor]?.[0],neighborIndex=STEMS.indexOf(neighborStem),neighborElement=stemResolution.elements[neighbor];if(neighborIndex>=0&&neighborIndex%2===0&&neighborElement===dayElement){if(!stemResolution.stamps[1].includes('近貼'))stemResolution.stamps[1].push('近貼');if(!stemResolution.stamps[neighbor].includes('近貼'))stemResolution.stamps[neighbor].push('近貼')}}}
  ...
  const rules={
    wood:{
      strong:[
        ...
        ()=>paired(stems[0],stems[1],'丁壬')&&paired(stems[2],stems[3],'丁壬')&&majorHas('寅卯')&&outerCount('寅卯辰未亥')>=2,
      ],
      middle:[
        ()=>stemCount('甲乙')>=3,
    ...
    fire:{
      strong:[
        ...
        ()=>paired(stems[0],stems[1],'戊癸')&&paired(stems[2],stems[3],'戊癸')&&majorHas('巳午')&&outerCount('寅巳午未戌')>=2,
      ],
      middle:[
        ()=>stemCount('丙丁')>=3,
    ...
    water:{
      strong:[
        ...
        ()=>paired(stems[0],stems[1],'丙辛')&&paired(stems[2],stems[3],'丙辛')&&majorHas('子亥')&&outerCount('子丑辰申亥')>=2,
      ],
      middle:[
        ()=>stemCount('壬癸')>=3,
...
function destinyTemperature(elementStrengths,majorBranches=[]){
  ...
  const wood=elementStrengths.wood==='弱'?0:elementStrengths.wood==='強'?(eitherStrong?2:bothWeak?0:1):(bothWeak?0:1);
  const fireValue={強:4,中:2,弱:0}[elementStrengths.fire]??0;
  const earthTable={
    強:{'強強':0,'強中':-1,'強弱':-2,'中強':1,'中中':0,'中弱':-1,'弱強':2,'弱中':1,'弱弱':0},
    中:{'強強':0,'強中':-1,'強弱':-1,'中強':1,'中中':0,'中弱':-1,'弱強':1,'弱中':1,'弱弱':0},
    弱:{'強強':0,'強中':0,'強弱':0,'中強':0,'中中':0,'中弱':0,'弱強':0,'弱中':0,'弱弱':0},
  };
  const earth=earthTable[elementStrengths.earth]?.[`${fire}${water}`]??0;
  const metal=elementStrengths.metal==='弱'?0:elementStrengths.metal==='強'?(eitherStrong?-2:bothWeak?0:-1):(bothWeak?0:-1);
  const waterValue={強:-4,中:-2,弱:0}[elementStrengths.water]??0;
  const branchTemperature=branch=>'巳午未申'.includes(branch)?2:'亥子丑寅'.includes(branch)?-2:0;
  const monthBranch=majorBranches.filter(Boolean).reduce((sum,branch)=>sum+branchTemperature(branch),0);
...
function useGodFromStrengths(elementStrengths,dayStem,monthBranch){
  ...
  const strongPositions=relative.map((element,index)=>elementStrengths[element]==='弱'?null:index).filter(index=>index!==null),key=strongPositions.join('');
  const chartBaseByPattern={
    '0':1,'1':6,'2':11,'3':16,'4':21,'1234':126,'12':46,'13':51,'14':56,'23':61,'24':66,'34':71,
    '234':76,'134':81,'124':86,'123':91,'01':26,'02':31,'03':36,'04':41,
...
function renderElementCircle(target,data,ariaPrefix='五行得点',options={}){
  ...
  target.innerHTML=`<svg viewBox="0 0 400 360" role="img" aria-label="${ariaPrefix}。日主は${labels[dayMaster]}。${scoreList.replace(/<[^>]+>/g,' ')}${temperature?`。推命気温${temperature.total}度`:''}${useGod?.label?`。用神${useGod.label}`:''}"><circle class="five-elements-orbit" cx="${cx}" cy="${cy}" r="${orbit}"/>${nodes}</svg><button class="five-elements-score-toggle" type="button" aria-expanded="false">点数を表示</button><div class="five-elements-details" hidden><div class="five-elements-scores">${scoreList}</div>${temperatureMarkup}${useGodMarkup}${strengthMarkup}${basisTarget?'':basisMarkup}</div>`;
...
function westernDateFromInput(value){
  const normalized=String(value||'').trim().replace(/[０-９]/g,char=>String.fromCharCode(char.charCodeAt(0)-0xfee0)).replace(/[年月.\-／－]/g,'/').replace(/日$/,'');
  const compact=normalized.match(/^(\d{4})(\d{2})(\d{2})$/),separated=normalized.match(/^(\d{1,4})\/(\d{1,2})\/(\d{1,2})$/),parts=compact||separated;
  if(!parts)return null;const y=Number(parts[1]),m=Number(parts[2]),d=Number(parts[3]);return validCalendarDate(y,m,d)?{y,m,d}:null;
}
function formattedWesternDate(date){return`${String(date.y).padStart(4,'0')}/${String(date.m).padStart(2,'0')}/${String(date.d).padStart(2,'0')}`}
function readWesternDateControl(){return westernDateFromInput(document.querySelector('#birthDate').value)}
function updateCalendarInputs(){
  ...
  if(system==='western'){
    if(previous!=='western'){try{const converted=westernDateFromCalendar(previous,document.querySelector('#eraYear').value,document.querySelector('#eraMonth').value,document.querySelector('#eraDay').value);dateInput.value=formattedWesternDate(converted)}catch{}}
    dateInput.hidden=false;dateInput.disabled=false;dateInput.required=true;eraInput.hidden=true;help.textContent='例：1977/02/01（数字8桁でも入力できます）';eraInput.dataset.system='western';return;
  }
...
let memberSavedSubjects=[],memberSubjectUsage={used:0,limit:0},compatibilitySelectedIds=[],compatibilityPrintTitle='';
const MEMBER_FEATURES={FIVE_ELEMENT_BALANCE:'five_element_balance',LUCK_CYCLES:'luck_cycles',ANNUAL_FORTUNE:'annual_fortune',SIX_PILLARS:'six_pillars',PDF_REPORT:'pdf_report',SAVED_SUBJECTS:'saved_subjects',CHANGE_EVIDENCE:'change_evidence',COMPATIBILITY:'compatibility'};
function memberFeatureVisibility(features=[]){const enabled=new Set(features);return{fiveElements:enabled.has(MEMBER_FEATURES.FIVE_ELEMENT_BALANCE),luckCycles:enabled.has(MEMBER_FEATURES.LUCK_CYCLES),annualFortune:enabled.has(MEMBER_FEATURES.ANNUAL_FORTUNE),sixPillars:enabled.has(MEMBER_FEATURES.SIX_PILLARS),pdfReport:enabled.has(MEMBER_FEATURES.PDF_REPORT),savedSubjects:enabled.has(MEMBER_FEATURES.SAVED_SUBJECTS),changeEvidence:enabled.has(MEMBER_FEATURES.CHANGE_EVIDENCE),compatibility:enabled.has(MEMBER_FEATURES.COMPATIBILITY)}}
function renderMemberPlanGuide(member,access){
  const guide=document.querySelector('#memberPlanGuide');if(!guide)return;
  const planId=member?.plan?.id||member?.planId||'',title=document.querySelector('#memberPlanGuideTitle'),description=document.querySelector('#memberPlanGuideDescription'),link=document.querySelector('#memberPlanGuideLink');
  if(planId==='free'){title.textContent='現在はフリープランです';description.textContent='五行サークル・五行得点・大運・年運・鑑定書PDFはスターターから、六柱推命・相性鑑定・命式保存はプレミアムで利用できます。';link.textContent='有料プランを見る';guide.hidden=false;return}
  if(planId==='starter'){title.textContent='現在はスタータープランです';description.textContent='六柱推命・相性鑑定・命式保存100件はプレミアムプランで利用できます。';link.textContent='プレミアムを見る';guide.hidden=false;return}
  guide.hidden=true;
}
function applyMemberFeatureAccess(member){
  const access=memberFeatureVisibility(member.features),targets=[['.five-elements-section',access.fiveElements],['.luck-section',access.luckCycles],['.annual-section',access.annualFortune],['.six-mode-section',access.sixPillars],['.compatibility-section',access.compatibility],['.pdf-mode-section',access.pdfReport],['#hiddenStemsToggle',access.changeEvidence],['#natalTransformationBasis',access.changeEvidence]];
  for(const[selector,visible]of targets)for(const element of document.querySelectorAll(selector))element.hidden=!visible;
  document.body.classList.toggle('member-hide-change-evidence',!access.changeEvidence);
  return access;
}
function memberMessage(message,error=false){const target=document.querySelector('#memberSubjectsMessage');if(!target)return;target.textContent=message;target.classList.toggle('is-error',error)}
function currentSubjectPayload(){
  const x=parseInput(),calendarSystem=document.querySelector('#calendarSystem').value,localSelect=document.querySelector('#localOffset'),localValue=localSelect.value,selectedAnnualYear=Number(document.querySelector('#sixYearInput')?.value)||new Date().getFullYear();
  return{displayName:x.name,calendarSystem,birthYear:x.y,birthMonth:x.m,birthDay:x.d,birthHour:x.h,birthMinute:x.min,birthTimeUnknown:x.unknown,sex:x.sex,birthplaceLabel:x.birthplace,localOffsetMinutes:x.localOffset,standardLongitude:x.standardEast,hemisphere:x.hemisphere,selectedAnnualYear,extraInput:{localOffsetValue:localValue,customOffset:document.querySelector('#customOffset').value||'',timezoneLongitude:document.querySelector('#timezoneLongitude').value}};
}
function subjectLabel(subject){const name=subject.display_name||'お名前未入力';return`${name}｜${subject.birth_year}.${subject.birth_month}.${subject.birth_day}`}
function memberUsageLabel(){const{used,limit}=memberSubjectUsage;return limit===null?`${used}件保存／上限なし`:`${used} / ${limit}件保存`}
function renderSavedSubjects(selectedId=''){
  const select=document.querySelector('#savedSubjectSelect'),query=document.querySelector('#savedSubjectSearch').value.trim().toLowerCase(),sort=document.querySelector('#savedSubjectSort').value;
  const visible=memberSavedSubjects.filter(subject=>subjectLabel(subject).toLowerCase().includes(query)).sort((a,b)=>sort==='name-asc'?String(a.display_name||'').localeCompare(String(b.display_name||''),'ja'):sort==='birth-asc'?(a.birth_year-b.birth_year)||(a.birth_month-b.birth_month)||(a.birth_day-b.birth_day):String(b.updated_at||'').localeCompare(String(a.updated_at||'')));
  select.replaceChildren(...(visible.length?visible.map(subject=>new Option(subjectLabel(subject),subject.id)):[new Option('保存した命式はありません','')]));
  select.value=selectedId&&visible.some(subject=>subject.id===selectedId)?selectedId:(visible[0]?.id||'');
  const noMatches=query&&!visible.length;memberMessage(noMatches?`「${query}」に一致する命式はありません。`:memberSavedSubjects.length?`${memberUsageLabel()}（表示 ${visible.length}件）`:'保存した命式はまだありません。');
  renderCompatibilityPicker();
  return select.value;
}
function savedSubjectReading(subject){
  const y=Number(subject.birth_year),m=Number(subject.birth_month),d=Number(subject.birth_day),unknown=Boolean(subject.birth_time_unknown),h=unknown?12:Number(subject.birth_hour??12),min=unknown?0:Number(subject.birth_minute??0),standardEast=Number(subject.standard_longitude)||135,localOffset=Number(subject.local_offset_minutes)||0,time=correctedBirthTime(y,m,d,h,min,localOffset,standardEast),x={y,m,d,h,min,unknown,...time,standardEast,sex:subject.sex||'女性',name:subject.display_name||'',birthplace:subject.birthplace_label||'未入力',hemisphere:subject.hemisphere||'north'};
  return{x,p:getPillars(x)};
}
function compatibilityYearOptions(centerYear=new Date().getFullYear()){const center=Math.max(51,Math.min(9949,Math.trunc(Number(centerYear))||new Date().getFullYear()));return Array.from({length:101},(_,index)=>center-50+index)}
function compatibilityYear(){const input=document.querySelector('#compatibilityYearInput');return Math.max(1,Math.min(9999,Math.trunc(Number(input?.value)||Number(document.querySelector('#sixYearInput')?.value)||new Date().getFullYear())))}
function reorderedIds(ids,id,direction){const result=[...ids],from=result.indexOf(id),to=from+direction;if(from<0||to<0||to>=result.length)return result;[result[from],result[to]]=[result[to],result[from]];return result}
function moveCompatibilitySelection(id,direction){compatibilitySelectedIds=reorderedIds(compatibilitySelectedIds,id,direction);renderCompatibilityPicker()}
function renderCompatibilityPicker(){
  const picker=document.querySelector('#compatibilityPicker');if(!picker)return;
  const available=new Set(memberSavedSubjects.map(subject=>subject.id));compatibilitySelectedIds=compatibilitySelectedIds.filter(id=>available.has(id));
  picker.innerHTML=memberSavedSubjects.length?memberSavedSubjects.map(subject=>{const order=compatibilitySelectedIds.indexOf(subject.id),selected=order>=0;return`<div class="compatibility-person${selected?' is-selected':''}"><label><input type="checkbox" value="${escapeHtml(subject.id)}"${selected?' checked':''}><span><strong>${escapeHtml(subject.display_name||'お名前未入力')}</strong><small>${subject.birth_year}.${subject.birth_month}.${subject.birth_day}　${escapeHtml(subject.sex||'')}</small></span></label>${selected?`<div class="compatibility-order"><em>${order+1}</em><button type="button" data-move="-1" data-id="${escapeHtml(subject.id)}" aria-label="${escapeHtml(subject.display_name||'お名前未入力')}を前へ"${order===0?' disabled':''}>←</button><button type="button" data-move="1" data-id="${escapeHtml(subject.id)}" aria-label="${escapeHtml(subject.display_name||'お名前未入力')}を後ろへ"${order===compatibilitySelectedIds.length-1?' disabled':''}>→</button></div>`:''}</div>`}).join(''):'<p>相性鑑定に使うには、先に命式を保存してください。</p>';
  for(const checkbox of picker.querySelectorAll('input[type="checkbox"]'))checkbox.onchange=()=>{if(checkbox.checked&&compatibilitySelectedIds.length>=4){checkbox.checked=false;document.querySelector('#compatibilityMessage').textContent='一度に選択できるのは4名までです。';return}compatibilitySelectedIds=checkbox.checked?[...compatibilitySelectedIds,checkbox.value]:compatibilitySelectedIds.filter(id=>id!==checkbox.value);renderCompatibilityPicker()};
  for(const moveButton of picker.querySelectorAll('[data-move]'))moveButton.onclick=()=>moveCompatibilitySelection(moveButton.dataset.id,Number(moveButton.dataset.move));
  renderCompatibilitySheet();
}
function compatibilityCardMarkup(subject,context){const x=context.reading.x,six=context.six,pre=six.luck.pre;return`<article class="compatibility-card"><header><strong>${escapeHtml(subject.display_name||'お名前未入力')} 様</strong><span>${x.y}.${x.m}.${x.d}　${escapeHtml(x.sex)}</span><small>${six.year}年　年運 ${six.annualValue.join('')}${pre?' ／ 立運前':` ／ 大運 ${six.luck.value.join('')}`}</small></header><div class="compatibility-pillars-stage"><div class="pillars six-pillars compatibility-pillars"></div></div><div class="five-elements-chart compatibility-elements"></div></article>`}
function renderCompatibilitySheet(){
  const sheet=document.querySelector('#compatibilitySheet'),message=document.querySelector('#compatibilityMessage'),button=document.querySelector('#compatibilityPrintButton');if(!sheet||!message||!button)return;
  const subjectsById=new Map(memberSavedSubjects.map(subject=>[subject.id,subject])),selected=compatibilitySelectedIds.map(id=>subjectsById.get(id)).filter(Boolean).slice(0,4),year=compatibilityYear();sheet.innerHTML='';
  for(const subject of selected){try{const reading=savedSubjectReading(subject),six=buildSixPillarContext(reading.x,reading.p,year),holder=document.createElement('div');holder.innerHTML=compatibilityCardMarkup(subject,{reading,six});const card=holder.firstElementChild;card.querySelector('.compatibility-pillars').innerHTML=pillarMarkup(six.model);sheet.append(card);renderElementCircle(card.querySelector('.compatibility-elements'),six.balance,'相性鑑定の五行得点')}catch{}}
  message.textContent=selected.length?`${year}年の六柱命式を${selected.length}名分表示しています。${selected.length<2?'比較する方をもう1名以上選択してください。':''}`:'相性鑑定に表示する方を選択してください。';button.disabled=selected.length<2;requestAnimationFrame(()=>{for(const card of sheet.querySelectorAll('.compatibility-card'))drawElementGroupOutlines(card.querySelector('.compatibility-pillars'))})
}
function printCompatibilitySheet(){if(compatibilitySelectedIds.length<2)return;compatibilityPrintTitle=document.title;document.title=`相性鑑定_${compatibilityYear()}年`;document.body.classList.add('printing-compatibility');requestAnimationFrame(()=>window.print())}
function setMemberSubjectControlsDisabled(disabled){for(const id of['savedSubjectSelect','savedSubjectSearch','savedSubjectSort','renameSubjectButton','deleteSubjectButton','saveSubjectButton'])document.querySelector(`#${id}`).disabled=disabled}
async function refreshSavedSubjects(selectedId='',autoLoad=false){
  try{const response=await fetch('/members/api/subjects',{headers:{Accept:'application/json'}}),data=await response.json();if(response.status===403){setMemberSubjectControlsDisabled(true);memberMessage('現在のプランでは命式保存を利用できません。');return}if(!response.ok||!data.ok)throw Error('保存一覧を取得できませんでした');memberSavedSubjects=data.subjects||[];memberSubjectUsage=data.usage||{used:memberSavedSubjects.length,limit:null};const renderedId=renderSavedSubjects(selectedId);if(autoLoad&&renderedId)await loadSelectedSubject()}catch(error){memberMessage(error.message,true)}
}
function applySavedSubject(subject){
  document.querySelector('#name').value=subject.display_name||'';
  document.querySelector(`input[name=sex][value="${subject.sex}"]`).checked=true;
  const system=subject.calendar_system||'western',dateValue=formattedWesternDate({y:subject.birth_year,m:subject.birth_month,d:subject.birth_day});
  document.querySelector('#calendarSystem').value='western';updateCalendarInputs();document.querySelector('#birthDate').value=dateValue;if(system!=='western'){document.querySelector('#calendarSystem').value=system;updateCalendarInputs()}
  document.querySelector('#birthTime').value=`${String(subject.birth_hour??12).padStart(2,'0')}:${String(subject.birth_minute??0).padStart(2,'0')}`;document.querySelector('#unknownTime').checked=Boolean(subject.birth_time_unknown);document.querySelector('#birthTime').disabled=Boolean(subject.birth_time_unknown);
  document.querySelector('#hemisphere').value=subject.hemisphere||'north';document.querySelector('#timezoneLongitude').value=String(-(Number(subject.standard_longitude)||135));
  const localSelect=document.querySelector('#localOffset'),savedLocal=String(subject.extra_input?.localOffsetValue??subject.local_offset_minutes);localSelect.value=[...localSelect.options].some(option=>option.value===savedLocal)?savedLocal:'custom';document.querySelector('#customOffset').value=String(subject.extra_input?.customOffset??subject.local_offset_minutes??0);document.querySelector('#customOffset').hidden=localSelect.value!=='custom';
  showForm();memberMessage(`${subjectLabel(subject)} を表示しました。`);document.querySelector('#baziForm').requestSubmit();
}
async function loadSelectedSubject(){const id=document.querySelector('#savedSubjectSelect').value;if(!id)return memberMessage('表示する命式を選択してください。',true);try{const response=await fetch(`/members/api/subjects/${encodeURIComponent(id)}`,{headers:{Accept:'application/json'}}),data=await response.json();if(!response.ok||!data.ok)throw Error('命式を表示できませんでした');applySavedSubject(data.subject)}catch(error){memberMessage(error.message,true)}}
function newSubject(){
  const form=document.querySelector('#baziForm');form.reset();document.querySelector('#name').value='';document.querySelector('#calendarSystem').value='western';document.querySelector('#birthDate').value='';document.querySelector('#birthTime').value='10:00';document.querySelector('#unknownTime').checked=false;document.querySelector('#birthTime').disabled=false;document.querySelector('#timezoneLongitude').value='-135';document.querySelector('#hemisphere').value='north';document.querySelector('#localOffset').value='25';document.querySelector('#customOffset').hidden=true;updateCalendarInputs();showForm();memberMessage('新しい方の情報を入力してください。');document.querySelector('#name').focus();
}
async function saveCurrentSubject(){try{const payload=currentSubjectPayload(),response=await fetch('/members/api/subjects',{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify(payload)}),data=await response.json();if(response.status===409)throw Error(`保存上限（${data.limit}件）に達しています。`);if(response.status===403)throw Error('現在のプランでは命式保存を利用できません。');if(!response.ok||!data.ok)throw Error('命式を保存できませんでした');await refreshSavedSubjects(data.subject.id);memberMessage(`${subjectLabel(data.subject)} を保存しました。`)}catch(error){memberMessage(error.message,true)}}
async function renameSelectedSubject(){const select=document.querySelector('#savedSubjectSelect'),id=select.value,subject=memberSavedSubjects.find(item=>item.id===id);if(!subject)return memberMessage('名前を変更する命式を選択してください。',true);const displayName=prompt('新しい保存名を入力してください。',subject.display_name||'');if(displayName===null)return;if(!displayName.trim())return memberMessage('保存名を入力してください。',true);try{const response=await fetch(`/members/api/subjects/${encodeURIComponent(id)}`,{method:'PATCH',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({displayName})}),data=await response.json();if(!response.ok||!data.ok)throw Error('保存名を変更できませんでした');await refreshSavedSubjects(id);memberMessage(`${subjectLabel(data.subject)} に変更しました。`)}catch(error){memberMessage(error.message,true)}}
async function deleteSelectedSubject(){const select=document.querySelector('#savedSubjectSelect'),id=select.value,subject=memberSavedSubjects.find(item=>item.id===id);if(!subject)return memberMessage('削除する命式を選択してください。',true);if(!confirm(`「${subjectLabel(subject)}」を削除しますか？\nこの操作は元に戻せません。`))return;try{const response=await fetch(`/members/api/subjects/${encodeURIComponent(id)}`,{method:'DELETE',headers:{Accept:'application/json'}}),data=await response.json();if(!response.ok||!data.ok)throw Error('保存した命式を削除できませんでした');await refreshSavedSubjects();memberMessage(`「${subjectLabel(subject)}」を削除しました。`)}catch(error){memberMessage(error.message,true)}}
async function initMemberSubjects(){try{const response=await fetch('/members/api/session',{headers:{Accept:'application/json'}}),data=await response.json();if(!data.authenticated)return;const access=applyMemberFeatureAccess(data.member),price=data.member.plan.monthlyPrice?`月額${Number(data.member.plan.monthlyPrice).toLocaleString('ja-JP')}円`:'無料';renderMemberPlanGuide(data.member,access);document.querySelector('#memberSubjectsPlan').textContent=`${data.member.displayName||data.member.email} さん／${data.member.plan.label}（${price}）`;if(access.compatibility){const yearInput=document.querySelector('#compatibilityYearInput'),currentYear=new Date().getFullYear();yearInput.replaceChildren(...compatibilityYearOptions(currentYear).map(year=>new Option(`${year}年`,String(year))));yearInput.value=String(currentYear);yearInput.onchange=renderCompatibilitySheet;document.querySelector('#compatibilityPrintButton').onclick=printCompatibilitySheet}if(!access.savedSubjects)return;document.querySelector('#memberSubjects').hidden=false;document.querySelector('#saveSubjectButton').onclick=saveCurrentSubject;document.querySelector('#newSubjectButton').onclick=newSubject;document.querySelector('#savedSubjectSelect').onchange=loadSelectedSubject;document.querySelector('#renameSubjectButton').onclick=renameSelectedSubject;document.querySelector('#deleteSubjectButton').onclick=deleteSelectedSubject;document.querySelector('#savedSubjectSearch').oninput=()=>renderSavedSubjects(document.querySelector('#savedSubjectSelect').value);document.querySelector('#savedSubjectSort').onchange=()=>renderSavedSubjects(document.querySelector('#savedSubjectSelect').value);await refreshSavedSubjects('',true)}catch{}}
function init(){const tz=document.querySelector('#timezoneLongitude');TZ.forEach(([v,t])=>tz.add(new Option(t,v)));tz.value=-135;const lo=document.querySelector('#localOffset');CITIES.forEach(([v,n])=>{const option=new Option(`${n} ${v>0?'＋':v<0?'－':'±'}${Math.abs(v)}分`,v);option.dataset.location=n;lo.add(option)});lo.add(new Option('➡ 数値入力','custom'));lo.value=25;lo.onchange=()=>document.querySelector('#customOffset').hidden=lo.value!=='custom';document.querySelector('#calendarSystem').onchange=updateCalendarInputs;const birthDateInput=document.querySelector('#birthDate');birthDateInput.onblur=()=>{const date=readWesternDateControl();if(date)birthDateInput.value=formattedWesternDate(date)};updateCalendarInputs();document.querySelector('#unknownTime').onchange=e=>{document.querySelector('#birthTime').disabled=e.target.checked};document.querySelector('#baziForm').onsubmit=submit;document.querySelector('#editButton').onclick=document.querySelector('#backButton').onclick=showForm;document.querySelector('#hiddenStemsToggle').onclick=document.querySelector('#sixHiddenStemsToggle').onclick=toggleHiddenStems;document.querySelector('#pdfDownloadButton').onclick=printPdfReport;window.addEventListener('resize',()=>requestAnimationFrame(redrawAllElementOutlines));window.addEventListener('beforeprint',()=>{redrawPdfOutlines();for(const card of document.querySelectorAll('.compatibility-card'))drawElementGroupOutlines(card.querySelector('.compatibility-pillars'))});window.addEventListener('afterprint',()=>{document.body.classList.remove('printing-pdf-report','printing-compatibility');if(prePrintTitle)document.title=prePrintTitle;if(compatibilityPrintTitle){document.title=compatibilityPrintTitle;compatibilityPrintTitle=''}});initMemberSubjects()}
...
function parseInput(){const system=document.querySelector('#calendarSystem').value,date=system==='western'?readWesternDateControl():westernDateFromCalendar(system,document.querySelector('#eraYear').value,document.querySelector('#eraMonth').value,document.querySelector('#eraDay').value);if(!date)throw Error('生年月日を入力してください');const {y,m,d}=date;if(y<1||y>9999)throw Error('西暦1年から9999年までで入力してください');const unknown=document.querySelector('#unknownTime').checked;const ts=document.querySelector('#birthTime').value||'12:00';const [h,min]=ts.split(':').map(Number);const locationInput=document.querySelector('#localOffset'),raw=locationInput.value;const local=raw==='custom'?Number(document.querySelector('#customOffset').value||0):Number(raw),birthplace=raw==='custom'?`地方時差 ${signedMinutes(local)}`:locationInput.selectedOptions[0]?.dataset.location||'未入力';const tzWest=Number(document.querySelector('#timezoneLongitude').value);const standardEast=-tzWest,time=correctedBirthTime(y,m,d,h,min,local,standardEast);return{y,m,d,h,min,unknown,...time,standardEast,sex:document.querySelector('input[name=sex]:checked').value,name:document.querySelector('#name').value.trim(),birthplace,hemisphere:document.querySelector('#hemisphere').value}}
function getPillars(x){const solarBasis=usesHistoricalCalendarBasis(x)?(x.calendarInstant||x.instant):x.instant,lon=solarLongitude(solarBasis),southern=x.hemisphere==='south';const beforeRisshun=x.m===1||(x.m===2&&lon<315);const solarYear=beforeRisshun?x.y-1:x.y;const yearIndex=mod(solarYear-1984,60);const yearStem=yearIndex%10,northernYearBranch=yearIndex%12;const monthNo=mod(Math.floor(mod(lon-315,360)/30),12);const northernMonthBranch=mod(2+monthNo,12);const monthStem=mod((yearStem%5)*2+2+monthNo,10),monthBranch=mod(northernMonthBranch+(southern?6:0),12),yearBranch=mod(northernYearBranch+(southern?6:0),12);const daySerial=Math.floor(utcDate(x.apparent.getUTCFullYear(),x.apparent.getUTCMonth()+1,x.apparent.getUTCDate()).getTime()/86400000);const epoch=Math.floor(utcDate(2001,1,1).getTime()/86400000);const dayIndex=mod(daySerial-epoch,60),dayStem=dayIndex%10,dayBranch=dayIndex%12;let hour=null;if(!x.unknown){const mins=x.apparent.getUTCHours()*60+x.apparent.getUTCMinutes(),hb=mod(Math.floor((mins+60)/120),12),hourDayStem=mins>=23*60?mod(dayStem+1,10):dayStem;hour=[STEMS[mod(hourDayStem*2+hb,10)],BRANCHES[hb]]}return{hour,day:[STEMS[dayStem],BRANCHES[dayBranch]],month:[STEMS[monthStem],BRANCHES[monthBranch]],year:[STEMS[yearStem],BRANCHES[yearBranch]]}}
function submit(e){e.preventDefault();try{const x=parseInput(),p=getPillars(x);render(x,p)}catch(err){alert(err.message)}}
function render(x,p){activeReading={x,p};document.querySelector('#formView').hidden=true;document.querySelector('#resultView').hidden=false;document.querySelector('#resultName').textContent=`${x.name||'お名前未入力'} さん`;document.querySelector('#resultMeta').textContent=`${x.y}.${x.m}.${x.d}${x.unknown?'　出生時刻不明':`　${String(x.h).padStart(2,'0')}:${String(x.min).padStart(2,'0')}`}　${x.sex}`;document.querySelector('#pillars').innerHTML=pillarMarkup(originalPillarModel(p));const correction=`地方時差 ${signedMinutes(x.localOffset)}・均時差 ${signedMinutes(x.equationOffset)}${x.summerTimeCorrection?`・サマータイム ${signedMinutes(x.summerTimeCorrection)}`:''}`;document.querySelector('#solarTime').textContent=x.unknown?'時刻不明のため、時柱は表示していません':`視太陽時　${x.apparent.getUTCFullYear()}年${x.apparent.getUTCMonth()+1}月${x.apparent.getUTCDate()}日 ${String(x.apparent.getUTCHours()).padStart(2,'0')}:${String(x.apparent.getUTCMinutes()).padStart(2,'0')}（${correction}）`;renderFiveElementsCircle(p);renderLuck(x,p);renderSixMode(x,p);renderAnnual(x,p,document.querySelector('#sixYearInput')?.value);renderPdfMode(x,p);requestAnimationFrame(redrawAllElementOutlines);document.fonts?.ready.then(redrawAllElementOutlines);scrollTo({top:0,behavior:'smooth'})}
...
function annualWindowYears(year){const selectedYear=Math.trunc(Number(year));return Array.from({length:11},(_,i)=>selectedYear-5+i)}
function renderAnnual(x,p,year=new Date().getFullYear()){const selectedYear=Math.trunc(Number(year))||new Date().getFullYear(),years=annualWindowYears(selectedYear),luck=getLuckCycles(x,p),natalValues=[p.hour,p.day,p.month,p.year],natal=resolveNatalFiveElements(p),values=years.map(annualPillarForYear),resolutions=values.map((value,index)=>{const active=annualRelationLuckForYear(luck,years[index]),luckValue=active?[active.stem,active.branch]:null,luckResolution=luckValue?resolveDownstreamPillar(natal.states,natalValues,luckValue,'major',p.month[1]):null,contextStates=luckResolution?[...natal.states,luckResolution.state]:natal.states,contextValues=luckValue?[...natalValues,luckValue]:natalValues;return resolveDownstreamPillar(contextStates,contextValues,value,'minor',p.month[1])}),models=resolutions.map(downstreamPillarColumn),luckChanges=new Map(luck.cycles.map(c=>[c.start.year,c])),actualCurrentYear=new Date().getFullYear();document.querySelector('#annualSummary').textContent=`${selectedYear}年を中心に前後5年`;document.querySelector('#annualGrid').innerHTML=years.map((year,column)=>{const selected=year===selectedYear,change=luckChanges.get(year),marks=`${selected?`<span class="annual-badge">${selectedYear===actualCurrentYear?'今年':'選択年'}</span>`:''}${change?`<span class="annual-luck-badge">大運 ${change.stem}${change.branch}</span>`:''}`;return`<article class="annual-card ${selected?'is-current':''} ${(selected||change)?'has-mark':''}">${marks?`<div class="annual-marks">${marks}</div>`:''}<div class="annual-year">${year}</div><div class="annual-kanji cycle-kanji">${models[column].cells.map(cycleCellMarkup).join('')}</div></article>`}).join('')}
function renderSixMode(x,p,year){const input=document.querySelector('#sixYearInput');if(!input)return;const options=sixYearOptions(x,p),lastYear=options[options.length-1].year,selected=year??Math.min(lastYear,Math.max(x.y,new Date().getFullYear())),context=buildSixPillarContext(x,p,selected),age=context.year-x.y,pre=context.luck.pre;input.innerHTML=options.map(option=>`<option value="${option.year}">${option.label}</option>`).join('');input.value=String(context.year);document.querySelector('#sixModeSummary').textContent=`${context.year}年（${age}歳）年運 ${context.annualValue.join('')}${pre?'　｜　立運前':`　｜　大運 ${context.luck.value.join('')}`}`;document.querySelector('#sixModeDescription').textContent=pre?'原命式＋年運の十文字（立運前）':'原命式＋大運＋年運の十二文字';document.querySelector('#sixElementsTitle').textContent=pre?'十文字の五行サークル':'十二文字の五行サークル';document.querySelector('#sixElementsDescription').textContent=pre?'原命式・年運を合わせた五行バランス':'原命式・大運・年運を合わせた五行バランス';document.querySelector('#sixPillars').innerHTML=pillarMarkup(context.model);renderElementCircle('#sixElementsChart',context.balance,`${pre?'十':'十二'}文字の五行得点`,{basisTarget:'#sixTransformationBasis'});input.onchange=()=>{renderSixMode(x,p,input.value);renderAnnual(x,p,input.value);renderPdfMode(x,p,input.value)};requestAnimationFrame(()=>drawElementGroupOutlines('#sixPillars'))}
function pdfFortuneCycleContext(x,p,year){
  ...
  const years=annualWindowYears(selectedYear),annualValues=years.map(annualPillarForYear);
