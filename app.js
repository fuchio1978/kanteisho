...
function applyTransformationDemands(states,demands,budgets,priority,label){
  const usable=demands.filter(demand=>demand.amount>0&&budgets[demand.source]>0&&eligibleFlexibleAmount(states[demand.target],priority)>0);
  ...
  for(const [source,items] of bySource){let remaining=budgets[source];items.sort((a,b)=>(a.priority??priority)-(b.priority??priority)||a.target-b.target);for(const item of items){item.provisional=Math.min(item.amount,remaining);if(item.consumeSource!==false)remaining-=item.provisional}}
  ...
  for(const [target,items] of byTarget){const state=states[target],capacity=eligibleFlexibleAmount(state,priority),sourceTier=source=>source<4?0:source===4?1:2,tierGroups=new Map();
    for(const item of items){const tier=sourceTier(item.source);if(!tierGroups.has(tier))tierGroups.set(tier,[]);tierGroups.get(tier).push(item)}
...
function applyNatalBranchTransformations(states,stemScores,monthBranch,stemElements,options={}){
  ...
  for(const [a,b] of pairs){
    ...
    if(firstPower>=5&&secondPower<=1){second.canceled=true;second.transformations.push('冲・抜根')}
    else if(secondPower>=5&&firstPower<=1){first.canceled=true;first.transformations.push('冲・抜根')}
  }
  ...
  function collectHalf(entries,priority,label,requiresCenter=false){
    ...
    for(const [element,chars] of entries){
      ...
      for(const source of sourceIndices)for(const target of targetIndices){
        ...
        demands.push({source,target,element,amount:distance(source,target),priority,label,consumeSource:nonConsuming?false:undefined,dualEarth:element==='fire'&&['未','戌'].includes(states[target].char)});
      }
    }
    return demands;
  }
  const directionDemands=collectHalf(FORMATIONS.direction,7,'半方合'),meetingDemands=collectHalf(FORMATIONS.meeting,8,'半会',true);
  const appliedHalf=[...applyTransformationDemands(states,directionDemands,budgets,7,'半方合'),...applyTransformationDemands(states,meetingDemands,budgets,8,'半会')];
  ...
  for(const [a,b] of pairs){
    ...
    if((['巳','午'].includes(first.char)&&['丑','辰'].includes(second.char))||(['巳','午'].includes(second.char)&&['丑','辰'].includes(first.char))){
      const source=['巳','午'].includes(first.char)?a:b,target=source===a?b:a;soilDemands.push({source,target,element:'earth',amount:1,sourceOverlay:'earth',consumeSource:false});
    }else if((first.char==='未'&&['丑','辰','戌'].includes(second.char))||(second.char==='未'&&['丑','辰','戌'].includes(first.char))){
      const source=first.char==='未'?a:b,target=source===a?b:a;if(states[source].role==='major')soilDemands.push({source,target,element:'earth',amount:1});
    }
    else if(key==='巳戌'){const source=first.char==='巳'?a:b,target=first.char==='戌'?a:b;soilDemands.push({source,target,element:'earth',amount:1});}
  }
...
const HIDDEN_STEM_GROUPS={子:[['壬','癸']],丑:[['癸'],['辛'],['己']],寅:[['戊'],['丙'],['甲']],卯:[['甲','乙']],辰:[['乙'],['癸'],['戊']],巳:[['戊'],['庚'],['戊']],午:[['丙','丁'],['己']],未:[['丁'],['乙'],['己']],申:[['壬'],['庚']],酉:[['庚','辛']],戌:[['辛'],['丁'],['戊']],亥:[['甲'],['壬']]};
...
function renderElementCircle(target,data,ariaPrefix='五行得点'){
...
function renderFiveElementsCircle(p){renderElementCircle('#fiveElementsChart',natalElementScores(p),'原命式の五行得点')}
...
function sixYearOptions(x,p){return Array.from({length:Math.min(120,9999-x.y)+1},(_,age)=>{const year=x.y+age,luck=selectedLuckForYear(x,p,year),luckLabel=luck.pre?'立運前':luck.value.join('');return{year,age,luck,label:`${year}年／${age}歳／${luckLabel}`}})}
...
function parseInput(){const ds=document.querySelector('#birthDate').value;if(!ds)throw Error('生年月日を入力してください');const [y,m,d]=ds.split('-').map(Number);if(y<1||y>9999)throw Error('西暦1年から9999年までで入力してください');const unknown=document.querySelector('#unknownTime').checked;const ts=document.querySelector('#birthTime').value||'12:00';const [h,min]=ts.split(':').map(Number);const raw=document.querySelector('#localOffset').value;const local=raw==='custom'?Number(document.querySelector('#customOffset').value||0):Number(raw);const tzWest=Number(document.querySelector('#timezoneLongitude').value);const standardEast=-tzWest,time=correctedBirthTime(y,m,d,h,min,local,standardEast);return{y,m,d,h,min,unknown,...time,standardEast,sex:document.querySelector('input[name=sex]:checked').value,name:document.querySelector('#name').value.trim(),hemisphere:document.querySelector('#hemisphere').value}}
...
function renderLuck(x,p){const luck=getLuckCycles(x,p),values=luck.cycles.map(c=>[c.stem,c.branch]),natalValues=[p.hour,p.day,p.month,p.year],natal=resolveNatalFiveElements(p),states=values.map(value=>resolveDownstreamBranch(natal.states,natalValues,value,'major',p.month[1]));const sy=Math.floor(luck.startMonths/12),sm=luck.startMonths%12,models=elementColumnsModel(values,{branchProfiles:states,connectHorizontal:false});document.querySelector('#luckSummary').textContent=`${luck.direction>0?'順行運':'逆行運'}　｜　立運 ${sy}歳${sm?`${sm}か月`:''}`;document.querySelector('#luckGrid').innerHTML=luck.cycles.map((c,index)=>`<article class="luck-card ${c.current?'is-current':''}">${c.current?'<span class="current-badge">現在</span>':''}<div class="luck-age">${c.ageYears}歳${c.ageMonths?`${c.ageMonths}か月`:''}〜</div><div class="luck-date">${c.start.year}.${String(c.start.month).padStart(2,'0')}</div><div class="luck-kanji">${models[index].cells.map(cell=>`<span class="${originalCellClasses(cell)}"${originalCellAttributes(cell)}>${cell.char}</span>`).join('')}</div></article>`).join('')}
function renderAnnual(x,p){const currentYear=new Date().getFullYear();const years=Array.from({length:11},(_,i)=>currentYear-5+i),luck=getLuckCycles(x,p),natalValues=[p.hour,p.day,p.month,p.year],natal=resolveNatalFiveElements(p);const values=years.map(year=>{const index=mod(year-1984,60);return[STEMS[index%10],BRANCHES[index%12]]});const states=values.map((value,index)=>{const year=years[index],active=[...luck.cycles].reverse().find(cycle=>cycle.start.year<=year),luckValue=active?[active.stem,active.branch]:null,luckState=luckValue?resolveDownstreamBranch(natal.states,natalValues,luckValue,'major',p.month[1]):null;return resolveDownstreamBranch(luckState?[...natal.states,luckState]:natal.states,luckValue?[...natalValues,luckValue]:natalValues,value,'minor',p.month[1])});const models=elementColumnsModel(values,{branchProfiles:states,connectHorizontal:false});const luckChanges=new Map(luck.cycles.map(c=>[c.start.year,c]));document.querySelector('#annualGrid').innerHTML=years.map((year,column)=>{const current=year===currentYear;const change=luckChanges.get(year);const marks=`${current?'<span class="annual-badge">今年</span>':''}${change?`<span class="annual-luck-badge">大運 ${change.stem}${change.branch}</span>`:''}`;return`<article class="annual-card ${current?'is-current':''} ${(current||change)?'has-mark':''}">${marks?`<div class="annual-marks">${marks}</div>`:''}<div class="annual-year">${year}</div><div class="annual-kanji">${models[column].cells.map(cell=>`<span class="${originalCellClasses(cell)}"${originalCellAttributes(cell)}>${cell.char}</span>`).join('')}</div></article>`}).join('')}
function renderSixMode(x,p,year){const input=document.querySelector('#sixYearInput');if(!input)return;const options=sixYearOptions(x,p),lastYear=options[options.length-1].year,selected=year??Math.min(lastYear,Math.max(x.y,new Date().getFullYear())),context=buildSixPillarContext(x,p,selected),age=context.year-x.y,pre=context.luck.pre;input.innerHTML=options.map(option=>`<option value="${option.year}">${option.label}</option>`).join('');input.value=String(context.year);document.querySelector('#sixModeSummary').textContent=`${context.year}年（${age}歳）年運 ${context.annualValue.join('')}${pre?'　｜　立運前':`　｜　大運 ${context.luck.value.join('')}`}`;document.querySelector('#sixModeDescription').textContent=pre?'原命式＋年運の十文字（立運前）':'原命式＋大運＋年運の十二文字';document.querySelector('#sixElementsTitle').textContent=pre?'十文字の五行サークル':'十二文字の五行サークル';document.querySelector('#sixElementsDescription').textContent=pre?'原命式・年運を合わせた五行バランス':'原命式・大運・年運を合わせた五行バランス';document.querySelector('#sixPillars').innerHTML=pillarMarkup(context.model);renderElementCircle('#sixElementsChart',context.balance,`${pre?'十':'十二'}文字の五行得点`);input.onchange=()=>{renderSixMode(x,p,input.value);renderPdfMode(x,p,input.value)};requestAnimationFrame(()=>drawElementGroupOutlines('#sixPillars'))}
function pdfReportContext(x,p,year){const six=buildSixPillarContext(x,p,year);return{year:six.year,age:six.year-x.y,pre:six.luck.pre,annualValue:six.annualValue,luck:six.luck,natalModel:originalPillarModel(p),natalBalance:natalElementScores(p),sixModel:six.model,sixBalance:six.balance}}
...
function renderPdfMode(x,p,year){const input=document.querySelector('#pdfYearInput'),audienceInput=document.querySelector('#pdfAudienceInput');if(!input)return;const options=sixYearOptions(x,p),fallback=document.querySelector('#sixYearInput')?.value||Math.min(options.at(-1).year,Math.max(x.y,new Date().getFullYear())),context=pdfReportContext(x,p,year??fallback),studentMode=audienceInput?.value==='student',report=document.querySelector('#pdfReport');input.innerHTML=options.map(option=>`<option value="${option.year}">${option.label}</option>`).join('');input.value=String(context.year);report.classList.toggle('pdf-training',studentMode);document.querySelector('#pdfReportType').textContent='講座生添削用';document.querySelector('#pdfReportName').textContent=`${x.name||'お名前未入力'} 様`;document.querySelector('#pdfReportMeta').textContent=`${x.y}年${x.m}月${x.d}日生　${x.sex}`;document.querySelector('#pdfOriginalPillars').innerHTML=pillarMarkup(context.natalModel);document.querySelector('#pdfSixPillars').innerHTML=pillarMarkup(context.sixModel);document.querySelector('#pdfSixTitle').textContent=context.pre?'指定年の十文字':'指定年の十二文字';document.querySelector('#pdfSixElementsTitle').textContent=context.pre?'十文字の五行バランス':'十二文字の五行バランス';document.querySelector('#pdfSixSummary').textContent=`${context.year}年（${context.age}歳） 年運 ${context.annualValue.join('')}${context.pre?' ／ 立運前':` ／ 大運 ${context.luck.value.join('')}`}`;document.querySelector('#pdfReportFooter').textContent=`鑑定年 ${context.year}年${studentMode?' ／ 講座生添削用':''}`;renderElementCircle('#pdfOriginalElements',context.natalBalance,'原命式の五行得点');renderElementCircle('#pdfSixElements',context.sixBalance,`${context.pre?'十':'十二'}文字の五行得点`);preparePdfElementDetails('#pdfOriginalElements',studentMode);preparePdfElementDetails('#pdfSixElements',studentMode);input.onchange=()=>{renderPdfMode(x,p,input.value);const sixInput=document.querySelector('#sixYearInput');if(sixInput){sixInput.value=input.value;renderSixMode(x,p,input.value)}};if(audienceInput)audienceInput.onchange=()=>renderPdfMode(x,p,input.value);requestAnimationFrame(redrawPdfOutlines)}
...
function toggleHiddenStems(){const hidden=document.body.classList.toggle('hide-hidden-stems');for(const button of document.querySelectorAll('.hidden-stems-toggle')){button.textContent=hidden?'蔵干を表示':'蔵干を非表示';button.setAttribute('aria-pressed',String(!hidden))}requestAnimationFrame(redrawAllElementOutlines)}
