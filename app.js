...
function renderSavedSubjects(selectedId=''){
  ...
  select.replaceChildren(new Option('保存した命式を選択',''),...visible.map(subject=>new Option(subjectLabel(subject),subject.id)));
  if(selectedId&&visible.some(subject=>subject.id===selectedId))select.value=selectedId;
...
function setMemberSubjectControlsDisabled(disabled){for(const id of['savedSubjectSelect','savedSubjectSearch','savedSubjectSort','loadSubjectButton','renameSubjectButton','deleteSubjectButton','saveSubjectButton'])document.querySelector(`#${id}`).disabled=disabled}
async function refreshSavedSubjects(selectedId=''){
  try{const response=await fetch('/members/api/subjects',{headers:{Accept:'application/json'}}),data=await response.json();if(response.status===403){setMemberSubjectControlsDisabled(true);memberMessage('現在のプランでは命式保存を利用できません。');return}if(!response.ok||!data.ok)throw Error('保存一覧を取得できませんでした');memberSavedSubjects=data.subjects||[];memberSubjectUsage=data.usage||{used:memberSavedSubjects.length,limit:null};renderSavedSubjects(selectedId)}catch(error){memberMessage(error.message,true)}
}
function applySavedSubject(subject){
  ...
  showForm();memberMessage(`${subjectLabel(subject)} を入力欄へ呼び戻しました。`);document.querySelector('#baziForm').requestSubmit();
}
async function loadSelectedSubject(){const id=document.querySelector('#savedSubjectSelect').value;if(!id)return memberMessage('呼び戻す命式を選択してください。',true);try{const response=await fetch(`/members/api/subjects/${encodeURIComponent(id)}`,{headers:{Accept:'application/json'}}),data=await response.json();if(!response.ok||!data.ok)throw Error('命式を呼び戻せませんでした');applySavedSubject(data.subject)}catch(error){memberMessage(error.message,true)}}
...
async function initMemberSubjects(){try{const response=await fetch('/members/api/session',{headers:{Accept:'application/json'}}),data=await response.json();if(!data.authenticated)return;document.querySelector('#memberSubjects').hidden=false;document.querySelector('#memberSubjectsPlan').textContent=`${data.member.displayName||data.member.email} さん／${data.member.planId}プラン`;document.querySelector('#saveSubjectButton').onclick=saveCurrentSubject;document.querySelector('#loadSubjectButton').onclick=loadSelectedSubject;document.querySelector('#renameSubjectButton').onclick=renameSelectedSubject;document.querySelector('#deleteSubjectButton').onclick=deleteSelectedSubject;document.querySelector('#savedSubjectSearch').oninput=()=>renderSavedSubjects(document.querySelector('#savedSubjectSelect').value);document.querySelector('#savedSubjectSort').onchange=()=>renderSavedSubjects(document.querySelector('#savedSubjectSelect').value);await refreshSavedSubjects()}catch{}}
