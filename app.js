...
let memberSavedSubjects=[],memberSubjectUsage={used:0,limit:0},compatibilitySelectedIds=new Set(),compatibilityPrintTitle='';
...
function renderCompatibilityPicker(){
  ...
  const available=new Set(memberSavedSubjects.map(subject=>subject.id));compatibilitySelectedIds=new Set([...compatibilitySelectedIds].filter(id=>available.has(id)));
  picker.innerHTML=memberSavedSubjects.length?memberSavedSubjects.map(subject=>`<label><input type="checkbox" value="${escapeHtml(subject.id)}"${compatibilitySelectedIds.has(subject.id)?' checked':''}><span><strong>${escapeHtml(subject.display_name||'お名前未入力')}</strong><small>${subject.birth_year}.${subject.birth_month}.${subject.birth_day}　${escapeHtml(subject.sex||'')}</small></span></label>`).join(''):'<p>相性鑑定に使うには、先に命式を保存してください。</p>';
  for(const checkbox of picker.querySelectorAll('input[type="checkbox"]'))checkbox.onchange=()=>{if(checkbox.checked&&compatibilitySelectedIds.size>=4){checkbox.checked=false;document.querySelector('#compatibilityMessage').textContent='一度に選択できるのは4名までです。';return}checkbox.checked?compatibilitySelectedIds.add(checkbox.value):compatibilitySelectedIds.delete(checkbox.value);renderCompatibilitySheet()};
  renderCompatibilitySheet();
}
function compatibilityCardMarkup(subject,context){const x=context.reading.x,six=context.six,pre=six.luck.pre;return`<article class="compatibility-card"><header><strong>${escapeHtml(subject.display_name||'お名前未入力')} 様</strong><span>${x.y}.${x.m}.${x.d}　${escapeHtml(x.sex)}</span><small>${six.year}年　年運 ${six.annualValue.join('')}${pre?' ／ 立運前':` ／ 大運 ${six.luck.value.join('')}`}</small></header><div class="pillars six-pillars compatibility-pillars"></div><div class="five-elements-chart compatibility-elements"></div></article>`}
function renderCompatibilitySheet(){
  ...
  const selected=memberSavedSubjects.filter(subject=>compatibilitySelectedIds.has(subject.id)).slice(0,4),year=compatibilityYear();sheet.innerHTML='';
...
function printCompatibilitySheet(){if(compatibilitySelectedIds.size<2)return;compatibilityPrintTitle=document.title;document.title=`相性鑑定_${compatibilityYear()}年`;document.body.classList.add('printing-compatibility');requestAnimationFrame(()=>window.print())}
