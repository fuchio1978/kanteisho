...
async function refreshSavedSubjects(){
  const select=document.querySelector('#savedSubjectSelect');
  try{const response=await fetch('/members/api/subjects',{headers:{Accept:'application/json'}}),data=await response.json();if(response.status===403){select.disabled=true;document.querySelector('#loadSubjectButton').disabled=true;document.querySelector('#saveSubjectButton').disabled=true;memberMessage('現在のプランでは命式保存を利用できません。');return}if(!response.ok||!data.ok)throw Error('保存一覧を取得できませんでした');select.innerHTML='<option value="">保存した命式を選択</option>'+data.subjects.map(subject=>`<option value="${subject.id}">${subjectLabel(subject)}</option>`).join('');memberMessage(data.subjects.length?`${data.subjects.length}件の命式を保存しています。`:'保存した命式はまだありません。')}catch(error){memberMessage(error.message,true)}
}
