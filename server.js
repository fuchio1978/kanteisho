...
const {publicMemberReadiness, authenticateMember, listSavedSubjects, getSavedSubject, countSavedSubjects, createSavedSubject, renameSavedSubject, deleteSavedSubject, listMemberUsage, updateMemberAccess, inviteMember, recordManualSubscription, getMemberSubscription, listManualSubscriptions, listAdminAuditLogs, updateManualSubscription, completeMemberInvite, requestMemberPasswordReset, resetMemberPassword} = require('./supabase-server');
...
function adminUsagePage(members = [], {member, subscriptions = [], auditLogs = [], message = '', error = false, warnings = [], storeReadiness = storesCatalogReadiness()} = {}) {
  ...
  const contractAttentionRows = subscriptions.map(subscription => {
    ...
    if (!reason) return '';
    return `<tr><td><strong>${escapeHtml(memberNames.get(subscription.member_user_id) || '会員不明')}</strong></td><td>${escapeHtml(getPlan(subscription.plan_id).label)}</td><td>${escapeHtml(dateInputValue(subscription.current_period_ends_at) || '未設定')}</td><td><span class="attention">${escapeHtml(reason)}</span></td></tr>`;
  }).filter(Boolean).join('');
...
async function handle(req, res, dependencies = {authenticateMember, listSavedSubjects, getSavedSubject, countSavedSubjects, createSavedSubject, renameSavedSubject, deleteSavedSubject, listMemberUsage, updateMemberAccess, inviteMember, recordManualSubscription, getMemberSubscription, listManualSubscriptions, listAdminAuditLogs, updateManualSubscription, completeMemberInvite, requestMemberPasswordReset, resetMemberPassword}) {
  ...
  if (req.method === 'GET' && url.pathname === '/members/admin') {
    ...
    const saved = url.searchParams.get('saved') === '1', contractSaved = url.searchParams.get('contractSaved') === '1', contractCreated = url.searchParams.get('contractCreated') === '1', invited = url.searchParams.get('invited') === '1', resetSent = url.searchParams.get('resetSent') === '1', failed = url.searchParams.has('error');
    ...
    const message = invited ? '招待メールを送信しました。お客さまがパスワードを設定すると利用中になります。' : resetSent ? 'パスワード再設定メールを送信しました。会員本人に最新メールをご確認いただいてください。' : contractCreated ? '購入情報を契約台帳へ登録し、会員の利用プランを更新しました。' : contractSaved ? '契約内容と会員の利用プランを更新しました。' : saved ? '会員のプランと利用状態を更新しました。次回ログインから反映されます。' : failed ? (errorMessages[errorCode] || '招待または変更を完了できませんでした。入力内容をご確認ください。') : '';
